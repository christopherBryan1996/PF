...
export default function Favorites() {
  ...
  const checkFavoritos = () => {
    if (
      authGoo.logNormal &&
      eventosFavoritos.favouritesEvents &&
  ...
  useEffect(() => {
    ...
    if (authGoo.logNormal) {
      newFavs.length
        ? combinarFavs(authGoo.logNormal.uid, newFavs, dispatch)
  ...
  const eventoQuitado = () =>
    toast.warning("Evento fue eliminado de tus favoritos!", {
      position: "top-center",
      autoClose: 5000,
  ...
  const deleteFavoriteEvent = async (id: any, eventid: any) => {
    await axios.patch(
      `${URLrequests}api/users/removefavourite/${id}/${eventid}`
    );
    dispatch(getFavorites(authGoo.logNormal.uid));
  ...
  const seCopio = () =>
    toast.success("El URL del evento se copio en tu teclado", {
      position: "top-center",
      autoClose: 5000,
      hideProgressBar: false,
      closeOnClick: true,
  ...
  return (
    <div>
      ...
      <div className="container container-favorites">
        ...
        {eventosFavoritos.favouritesEvents
          ? eventosFavoritos.favouritesEvents.map((e: any) => (
              <>
                  ...
                  <div className="card-footer">
                    ...
                    <div className="icon">
                      <ImBin
                        ...
                        fontSize="1.6em"
                        onClick={() =>
                          deleteFavoriteEvent(authGoo.logNormal.uid, e._id)
                        }
                      />
                    </div>
                  </div>
